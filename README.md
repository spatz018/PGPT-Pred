...
Input formats:
(a) blastp+hmmer (PGPTblhm):            genomic protein sequences in FASTA format (sorted by genomic location)
(b) IMG-KEGG-annoation Mapper (PGPTpy): genomic protein KEGG annotations (received by IMG Server, or customer format)
...
Pipelines applied:
I) Protein/KEGG Annotation/Mapping against PGPT ontology
(a) blastp+hmmer (PGPTblhm):            genomic protein sequences are aligned against proteins associated with the PGPT ontology and respective PFAM domain comparison is achieved by hmmer against the PFAM domains using pgpt_blhm.py
(b) IMG-KEGG-annoation Mapper (PGPTpy): KEGG annotations (one per protein only) are mapped against the PGPT ontology, using pgpt_comp_fun_ascii_v2.py

II) Pie Chart generation 
(a) based on blastp+hmmer results or all blast hits (ignoring pfam comparison) of PGPTs, by applying:
(b) based on KEGG-PGPT mapping, by applying: 

III) Krona Plot generation
(a) based on blastp+hmmer results or all blast hits (ignoring pfam comparison) of PGPTs, by applying:
(b) based on KEGG-PGPT mapping, by applying: 
